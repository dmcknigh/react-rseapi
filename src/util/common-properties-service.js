...
export const fetchNamespaces = authCtx => {
  ...
  const queryURL = getURL(authCtx.hostName, authCtx.port, authCtx.isSecure, '/commonproperties')
...
export const fetchContent = (namespace, pathInJSON, authCtx) => {
  const JWT = authCtx.token
  const contentURL = getURL(authCtx.hostName, authCtx.port, authCtx.isSecure, '/commonproperties/' + namespace + '?jsonPath=' + pathInJSON)
  try {
